...
class HipermecadoCommand extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'command:hipermecado';
    ...
    public function handle()
    {
            ...
            $this->crawlerSite('https://hipermercado.pt/pt/');
        }
        ...
        private function handleData(Crawler $crawler): void
        {
            ...
            $products = $crawler->filter('.style_product_default')->each(function ($node) {
                $productName = $node->filter('.product_name')->text();
                $productImage = $node->filter('.first-image')->attr('src');
